...
print(f"🤖 CLOUD FREE VERSION - Token: {BOT_TOKEN[:6] if BOT_TOKEN else 'NO'}... OpenAI: {'YES' if OPENAI_API_KEY else 'NO'}")
...
async def transcribe_voice(file_path: str) -> str:
    try:
        with open(file_path, "rb") as f:
            tr = await client.audio.transcriptions.create(model="whisper-1", file=f, language="en")
...
# Web server for Render free tier to keep alive
